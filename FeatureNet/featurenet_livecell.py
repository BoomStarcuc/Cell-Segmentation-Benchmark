...
def main():
    ...
    parser.add_argument('--exp-name', type=str, default='featurenet_train_on_all_types_livecell', 
    ...
    X_train2 = np.empty([len(X_train), 256, 256])
    for idx , conponent in enumerate(X_train):    
        X_train2[idx,...] = (conponent - conponent.min()) / (conponent.max() - conponent.min())
    X_train = X_train2

    X_val2 = np.empty([len(X_val), 256, 256])
    for idx , conponent in enumerate(X_val):    
        X_val2[idx,...] = (conponent - conponent.min()) / (conponent.max() - conponent.min())
    X_val = X_val2
