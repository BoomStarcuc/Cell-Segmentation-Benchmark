...
def main():
    ...
    parser.add_argument('--excelname', type=str, default='featurenet_livecell', help='save masks as tif and outlines as text file for ImageJ')           
    ...
    X_test2 = np.empty([len(X_test), 256, 256])
    for idx , conponent in enumerate(X_test):    
        X_test2[idx,...] = (conponent - conponent.min()) / (conponent.max() - conponent.min())
    X_test = X_test2
