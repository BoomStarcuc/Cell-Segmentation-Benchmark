...
class COCOEvaluator(DatasetEvaluator):
    ...
    def _eval_predictions(self, tasks, predictions):
        ...
        for task in sorted(tasks):
            coco_eval = (
                _evaluate_predictions_on_coco(self, 
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        metrics = {
            "bbox": ["AP", "AP50", "AP75", "APs", "APm", "APl", "Recall50", "Recall75"],
            "segm": ["AP", "AP50", "AP75", "APs", "APm", "APl", "Recall50", "Recall75"],
            "keypoints": ["AP", "AP50", "AP75", "APm", "APl", "Recall50", "Recall75"],
        }[iou_type]
...
def _evaluate_predictions_on_coco(self, coco_gt, coco_results, iou_type, kpt_oks_sigmas=None, use_fast_impl=False):
    """
...
#    print("_evaluate_predictions_on_coco")
    ...
    def summarize_2(self, all_prec=False):
            ...
            def _summarizeDets():
                ...
                stats[6] = _summarize(0, maxDets=self.params.maxDets[2], iouThr=.5)  #added
                stats[7] = _summarize(0, maxDets=self.params.maxDets[2], iouThr=.75)  #added
    ...
    recall_iou = np.round(np.array(rec_pre_iou), 4)
    ap_iou = np.round(np.array(pre_per_iou), 4)
    # self._logger.info(f"Precision and Recall per iou: {coco_eval.params.iouThrs}")
    # self._logger.info(np.round(np.array(pre_per_iou), 4))
    # self._logger.info(np.round(np.array(rec_pre_iou), 4))
