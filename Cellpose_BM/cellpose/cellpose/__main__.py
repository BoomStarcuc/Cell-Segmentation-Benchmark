...
def main():
    ...
    input_img_args.add_argument('--nchan_test', default=1, type=int, help='the number of channels when testing')
    ...
    output_args.add_argument('--excelname', type=str, help='save .xlsx excel filename')
    output_args.add_argument('--dataname', type=str, help='data name used in test')
    ...
    if len(args.dir)==0 and len(args.image_path)==0:
        ...
        if not args.train and not args.train_size:
            ...
            masks_true = []
            masks_pred = []
            for image_name, label_name in tqdm(zip(image_names[::200], label_names[::200]), file=tqdm_out):
                image = io.imread(image_name)[np.newaxis, ...]
                ...
                if label.max() != 0:
                    tissue_list.append(os.path.basename(image_name).split('_')[0])
                    masks_true.append(label)
                    masks_pred.append(masks)

            tissue_list =  np.array(tissue_list)
            masks_true = np.array(masks_true)
            masks_pred = np.array(masks_pred)
            assert len(tissue_list) == len(masks_true) == len(masks_pred)
            
            if args.dataname ==  "TissueNet":
            ...
            for tissue_idx ,tissue in enumerate(unique_list):
                ...
                for count, value in enumerate(idx):
                    if count ==0:
                        ap, tp, fp, fn= metrics.average_precision(masks_true[value], masks_pred[value], threshold=[0.5 , 0.55, 0.6 , 0.65, 0.7 , 0.75, 0.8 , 0.85, 0.9 , 0.95])
                    else:
                        ap2, tp2, fp2, fn2= metrics.average_precision(masks_true[value], masks_pred[value], threshold=[0.5 , 0.55, 0.6 , 0.65, 0.7 , 0.75, 0.8 , 0.85, 0.9 , 0.95])
                        tp+=tp2
                        fp+=fp2
                        fn+=fn2
            ...
            data_df.columns = ['mAP','AP50','AP75','AR50','AR75']
