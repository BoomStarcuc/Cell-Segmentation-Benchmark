...
class CellposeModel(UnetModel):
    ...
    def eval(self, x, batch_size=8, channels=None, channel_axis=None, 
        ...
        if isinstance(x, list) or x.squeeze().ndim==5:
            ...
            # print("before transforms models:", x.shape)   
            ...
            # print("after transforms models:", x.shape)
            if x.ndim < 4:
                x = x[np.newaxis,...]
                # print("enter x<4 models:", x.shape)
    ...
    def _run_cp(self, x, compute_masks=True, normalize=True, invert=False,
        ...
        if do_3D:
            ...
            for i in iterator:
                img = np.asarray(x[i])
                # print("img models:", img.shape)
                if normalize or invert:
                    img = transforms.normalize_img(img, invert=invert)
                # print("img models2:", img.shape)
                if rescale != 1.0:
                    img = transforms.resize_image(img, rsz=rescale)
                # print("img models3:", img.shape)
    ...
    def train(self, train_data, train_labels, train_files=None, 
        ...
        # print("flow",train_data[0].shape, train_flows[0].shape)
