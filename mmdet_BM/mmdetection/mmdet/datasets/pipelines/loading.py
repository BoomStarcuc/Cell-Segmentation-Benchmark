...
class LoadAnnotations:
    ...
    def _poly2mask(self, mask_ann, img_h, img_w):
        ...
        """
        # if len(mask_ann) == 2:
        #     print("mask_ann:", mask_ann, np.array(mask_ann).shape)
        
        # if len(mask_ann[0]) <= 4:
        #     print("mask_ann:", mask_ann, np.array(mask_ann).shape)
        # if isinstance(mask_ann, list):
        #     print("mask_ann is a list")
        # elif isinstance(mask_ann, np.ndarray):
        #     print("mask_ann is a numpy array")
        #     exit(0)
        # else:
        #     print("mask_ann is neither a list nor a numpy array")
        #     exit(0)
    ...
    def _load_masks(self, results):
        ...
        # print(results)
        # print("@@@@@@@@@@@", results['img_info']["file_name"], results['img_info']["id"])
        # print("_load_masks gt_masks:", gt_masks)
        # exit(0)
        if self.poly2mask:
...
@PIPELINES.register_module()
class LoadTIFImageFromFile(object):
    def __init__(self,
                 to_float32=True,
                 color_type='color',
                 file_client_args=dict(backend='disk'),
    ...
    def __call__(self, results):
        ...
        if results.get('img_prefix') is not None:
            ...
            filename = results['img_info']['filename']
        img_bytes =  tiff.imread(filename)
        img_bytes = np.array(img_bytes,dtype='int32')
        if len(img_bytes.shape) < 3:
            img = np.stack((img_bytes,)*3, axis=-1)
        else:
            img = img_bytes
        if self.to_float32:
            img = img.astype(np.float32)
...
@PIPELINES.register_module()
class LoadTIFImageFromFile2C(object):
    def __init__(self,
                 to_float32=True,
                 color_type='color',
                 file_client_args=dict(backend='disk'),
    ...
    def __call__(self, results):
        ...
        num_channels = 1 if len(img.shape) < 3 else img.shape[2]
        # print("@@@@@@@@@@@@@@@@@@:", img.shape, num_channels)#(256, 256, 2) 2
        
        results['img_norm_cfg'] = dict(
            mean=np.zeros(num_channels, dtype=np.float32),
            std=np.ones(num_channels, dtype=np.float32),
            to_rgb=False)
        # print("results['img_norm_cfg']:", results['img_norm_cfg']) #{'mean': array([0., 0.], dtype=float32), 'std': array([1., 1.], dtype=float32), 'to_rgb': False}
        # exit(0)
        return results
    def __repr__(self):
        repr_str = self.__class__.__name__
